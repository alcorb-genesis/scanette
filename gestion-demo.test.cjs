...
function setup(profileCode){const body={innerHTML:''},root={querySelector:()=>body,querySelectorAll:()=>[],dataset:{},style:{setProperty(){}},addEventListener(){}};const c={DeliveryPrint:require('./delivery-print.js'),document:{getElementById:()=>root},window:{addEventListener(){}},console};if(profileCode)c.Workspaces={create:()=>({sync(){},home:()=>'',person:()=>({code:profileCode}),people:()=>[]})};vm.createContext(c);vm.runInContext(source.replace(' restore(window.openai?.widgetState);',' globalThis.api={switchStore,handle,stock,extension,routing,calendarRows,replenishmentState,dueFor,creditFor,ttc,journalRows,views,render,readyQty,simulationStale,getState:()=>s}; restore(window.openai?.widgetState);'),c);return c.api;}
{
...
{const a=setup('M008'),before=a.getState().docs.length;a.handle('validate');const d=a.getState().docs.at(-1);assert.equal(d.seller,'M008');assert.ok(d.createdAt);assert.equal(a.getState().docs.length,before+1);assert.ok(a.views.stats().includes(d.id));a.handle('invoice:'+d.id);const html=a.views.storestats();assert.ok(html.includes('4 ventes uniques'));assert.ok(html.includes('pas des erreurs confirmées'));}

{const a=setup();a.handle('validate');const d=a.getState().docs.at(-1);assert.equal(d.printSnapshot.id,d.id);const frozen=JSON.stringify(d.printSnapshot),before=a.stock(0);a.handle('print-bl:'+d.id);a.handle('print-bl:'+d.id);assert.equal(a.stock(0),before);assert.equal(JSON.stringify(d.printSnapshot),frozen);}
{const a=setup();a.handle('x-portal-add:0');a.handle('x-portal-send');const d=a.getState().docs.at(-1),count=a.getState().docs.length;assert.equal(d.printSnapshot.source,'Portail client');assert.equal(d.stage,'À préparer');assert.ok(d.createdAt);a.handle('x-portal-send');assert.equal(a.getState().docs.length,count);}
{const a=setup(),before=a.stock(0),s=a.getState(),d=s.docs[0];s.deliveryTours=[{driver:'Test',status:'completed',stops:[{documents:[d.id],status:'delivered'}]}];d.stage='Remis au départ';assert.ok(a.views.dispatch().includes('Livré · Test'));assert.equal(a.stock(0),before);a.switchStore('landes');assert.ok(!a.getState().deliveryTours);a.switchStore('bellecave');assert.equal(a.getState().deliveryTours[0].stops[0].status,'delivered');}
