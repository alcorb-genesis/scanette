...
$('receiptSend').onclick=async()=>{if(busy||!currentUserId||!storageReady)return;const receiptId=$('receiptTarget').value,section=sections.find(s=>s.id===$('receiptPointage').value);if(!receiptId||!section){msg('Choisissez le pointage et la réception.');return;}
 ...
 if(previous&&JSON.stringify(previous.lines)===JSON.stringify(rows)){associatedId=receiptId;msg('Ce pointage est déjà associé : aucune pièce ajoutée deux fois.');return;}
 ...
 associatedId=receiptId;msg('Pointage associé à '+latest.content.supplier_name+' · '+latest.content.orders+'. Ouvrez Réception pour consulter les articles.');
...
$('receiptOpen').onclick=()=>{if(parent!==window)parent.postMessage({type:'alcorb-section',section:'receipts',receiptId:associatedId},location.origin);else location.href='application.html#receipts';};
