...
function csv(d){return '\ufeff'+[['Dossier','Employé','Date début','Marque','Gamme','Zone','Référence','Désignation','Emplacement','Quantité comptée','État','Code interne','Code fabricant'],...d.lines.map(r=>[d.id,d.employee,d.createdAt,r.brand,r.range,d.zone,r.reference,r.description,r.location,r.quantity,r.added?'Ajout pendant inventaire':r.quantity===null?'Non compté':'Compté',r.internal_barcode,r.manufacturer_barcode])].map(r=>r.map(cell).join(';')).join('\r\n');}
