(function(scope){
 ...
 function filter(entries,{query='',kind='garage',area='local',contacts=false,selected=null}={}){const words=norm(query).split(/\s+/).filter(Boolean);return entries.filter(e=>(!kind||e.kind===kind)&&(!area||area==='local'&&['64','40'].includes(e.department)||e.department===area)&&(!contacts||e.phone||e.email)&&(!selected||selected.includes(e.id))&&words.every(w=>e.search.includes(w)));}
